<template>
  <div class="container py-4">
    ...
    <div class="row g-4">
      ...
      <!-- Changed: Income Per Month -->
      <div class="col-md-6">
        <div class="card shadow-sm">
          <div class="card-body">
            <h5 class="card-title text-center">Income Per Month</h5>
            <canvas id="incomeChart"></canvas>
          </div>
        </div>
      </div>

      <!-- Changed: Reservations Per Month -->
      <div class="col-md-6">
        <div class="card shadow-sm">
          <div class="card-body">
            <h5 class="card-title text-center">Reservations Per Month</h5>
...
// --- Income (converted daily -> monthly)
const fetchIncome = async () => {
  try {
    ...
    raw.forEach(i => {
      ...
      monthlyData[month] = (monthlyData[month] || 0) + (Number(i.daily_income) || 0)
    })
    ...
    incomeChart = new Chart(document.getElementById("incomeChart"), {
      ...
      data: {
        labels,
        datasets: [{ label: "Monthly Income", data: values, backgroundColor: "rgba(255,165,0,0.6)" }]
      }
    })
  } catch (err) {
...
// --- Reservations (converted daily -> monthly)
const fetchReservations = async () => {
  try {
    ...
    raw.forEach(i => {
      const month = new Date(i.date).toLocaleString("default", { month: "short", year: "numeric" })
      monthlyData[month] = (monthlyData[month] || 0) + (Number(i.total_reservations) || 0)
    })
    ...
    reservationChart = new Chart(document.getElementById("reservationChart"), {
      ...
      data: {
        labels,
        datasets: [{ label: "Monthly Reservations", data: values, backgroundColor: "rgba(128,0,128,0.6)" }]
      }
    })
  } catch (err) {
